...
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd ) {
	...
	if ( client->sess.spectatorState != SPECTATOR_FOLLOW ) {
		...
		// begin q3as
		if (client->q3as.MOTDPrinted == qfalse) {
			as_printMOTD(ent, as_motd_spectator_duration.integer);
			client->q3as.MOTDPrinted = qtrue;
		}
		// end q3as
	}
...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while ( client->timeResidual >= 1000 ) {
		...
		if ( client->ps.powerups[PW_REGEN] ) {
			...
			if ( as_shouldDecayHealth(ent) ) {	// q3as
		...
		if ( as_shouldDecayArmor(ent) ) {	// q3as
...
//		G_Printf("serverTime >>>>>\n" );
	...
	if ( client->sess.sessionTeam == TEAM_SPECTATOR ) {
		...
		as_clientSpectatorTimer( client );	// q3as
		return;
	}
	...
	as_regenVitals(ent);		// q3as
	as_regenAmmo(ent);			// q3as
}
...
void ClientThink( int clientNum ) {
	...
	// begin q3as
	if ( ent->r.svFlags & SVF_BOT && as_kick_spectator_bots.integer ) {
		if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
			if ( ent->client->sess.spectatorState != SPECTATOR_SCOREBOARD ) {
				if ( ent->client->pers.enterTime + 5000 < level.time ) {
					as_debug("Auto-kicking bot '%s' because they're accidentally spectating\n", ent->client->pers.netname);
