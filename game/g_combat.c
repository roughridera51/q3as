...
void TossClientItems( gentity_t *self ) {
	...
	if ( weapon > WP_MACHINEGUN && weapon != WP_GRAPPLING_HOOK && 
		...
		// spawn the item
		// q3as adds the 'if' statement here
		if (!(as_mode.mask & AS_MM_NODROPWEAPONS))
	...
	if ( g_gametype.integer != GT_TEAM ) {
		...
		for ( i = 1 ; i < PW_NUM_POWERUPS ; i++ ) {
			if ( self->client->ps.powerups[ i ] > level.time ) {
				...
				// begin q3as
				if ( item->giType != IT_TEAM && as_mode.mask & AS_MM_NODROPPOWERUPS )
					continue;
				// end q3as
...
void TossClientCubes( gentity_t *self ) {
	...
	if( neutralObelisk ) {
		...
		//VectorClear( origin ) ;
		VectorCopy( self->s.pos.trBase, origin );  // q3as
	}
...
void TossClientPersistantPowerups( gentity_t *ent ) {
	...
	// begin q3as
	if (powerup->flags & FL_GIVEN_PERSISTENT)
		G_FreeEntity(powerup);
	// end q3as
}
#endif
...
char	*modNames[] = {
	...
	"MOD_CHAINGUN",
	"MOD_PROXIMITY_MINE",
	// q3as - synced this list with the one in bg_public.h
	"MOD_KAMIKAZE",
	"MOD_JUICED",
	// end q3as
#endif
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	origMOD = meansOfDeath;			// q3as
	as_translateMOD(&meansOfDeath);	// q3as
	...
	if (as_log_osp.integer == qfalse)	// q3as
		G_LogPrintf("Kill: %i %i %i: %s killed %s by %s\n", 
			killer, self->s.number, meansOfDeath, killerName, 
			self->client->pers.netname, obit );

	as_logOSP("Kill: %i %i %i: %s killed %s by %s %i\n", 
		killer, self->s.number, meansOfDeath, killerName, 
		self->client->pers.netname, obit, self->s.weapon );	// q3as

	meansOfDeath = origMOD;	// q3as
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	if ( !(dflags & DAMAGE_NO_PROTECTION) ) {
		...
		// begin q3as
		if ( targ == attacker && ( as_mode.mask & AS_MM_NOSELF ) )
			return;
		if ( targ == attacker && ( as_mode.mask & AS_MM_NOSELFWOPU ) &&
			!( client &&
			 ( client->ps.powerups[PW_SCOUT]   ||
			   client->ps.powerups[PW_GUARD]   ||
			   client->ps.powerups[PW_DOUBLER] ||
			   client->ps.powerups[PW_AMMOREGEN])))
			return;
		// end q3as
	}
