...
void Touch_Item (gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if ( as_log_mask.integer & AS_MT_ITEM ) // q3as
...
//		predict = qfalse;
	...
	as_modifyRespawn(ent, &respawn);		// q3as
...
void G_CheckTeamItems( void ) {
	...
	if( g_gametype.integer == GT_HARVESTER ) {
		...
		// q3as adds surrounding if
		if ( !as_headhunter.integer ) {
			ent = NULL;
...
#endif
	// begin q3as
	// Obelisk explosion requires rocket explosion; It's possible
	// to make a map have no rockets, so force the register ...
	if (g_gametype.integer == GT_OBELISK) {
		as_debug("Forcing register of Rocket Launcher because GT == GT_OBELISK\n");
