...
void QDECL G_Printf( const char *fmt, ... ) {
	...
	as_sendMessageToReferees(text);	// q3as
}
...
void G_InitGame( int levelTime, int randomSeed, int restart ) {
	...
	as_openDebugLog();				// q3as
	as_loadQ3ASMapRotation(qfalse);	// q3as
	...
	as_initMode(as_mode_number.integer);					// q3as
...
void CalculateRanks( void ) {
	...
	for ( i = 0 ; i < level.maxclients ; i++ ) {
		if ( level.clients[i].pers.connected != CON_DISCONNECTED ) {
			...
			as_level.timeSortedClients[level.numConnectedClients] = i;	// q3as
	...
	qsort( as_level.timeSortedClients, level.numConnectedClients,
		sizeof(as_level.timeSortedClients[0]), as_sortTimeScore );	// q3as
...
void ExitLevel (void) {
	...
	// q3as trap_SendConsoleCommand( EXEC_APPEND, "vstr nextmap\n" );
	as_serverNextmap();	// q3as
...
start = trap_Milliseconds();
	...
	as_checkConfigTimeout();	// q3as

	as_logTick();				// q3as

	as_checkTeams();			// q3as

	as_checkLogRotation();		// q3as

	as_execScheduledMap();	// q3as - THIS MUST BE THE LAST LINE OF THIS FUNCTION!
}
