...
===============
*/
// q3as static void G_LoadArenas( void ) {
void G_LoadArenas( void ) {		// q3as
	int			numdirs;
	vmCvar_t	arenasFile;
	char		filename[128];
	char		dirlist[1024];
...
// q3as	char netname[36];
	...
	for ( i=0 ; i< g_maxclients.integer ; i++ ) {
		...
		// q3as strcpy(netname, cl->pers.netname);
		// q3as Q_CleanStr(netname);
		// q3as trap_SendConsoleCommand( EXEC_INSERT, va("kick %s\n", netname) );
		trap_SendConsoleCommand( EXEC_INSERT, va("clientkick %i\n", cl->ps.clientNum) );	// q3as
		return qtrue;
	}
	return qfalse;
}
...
void Svcmd_AddBot_f( void ) {
	...
	if ( !name[0] ) {
		// q3as trap_Printf( "Usage: Addbot <botname> [skill 1-5] [team] [msec delay] [altname]\n" );
...
void Svcmd_BotList_f( void ) {
	...
	// q3as trap_Printf("^1name             model            aifile              funname\n");
	G_Printf("^1name             model            aifile              funname\n");	// q3as
	for (i = 0; i < g_numBots; i++) {
		...
		// q3as trap_Printf(va("%-16s %-16s %-20s %-20s\n", name, model, aifile, funname));
		G_Printf(va("%-16s %-16s %-20s %-20s\n", name, model, aifile, funname));	// q3as
	}
}
...
char *G_GetBotInfoByNumber( int num ) {
	if( num < 0 || num >= g_numBots ) {
		// q3as trap_Printf( va( S_COLOR_RED "Invalid bot number: %i\n", num ) );
