...
static void CG_Obituary( entityState_t *ent ) {
	...
	char		targetName[MAX_NAME_LENGTH + 2];	// q3as - 32 -> MAX_NAME_LENGTH + 2
	char		attackerName[MAX_NAME_LENGTH + 2];	// q3as - 32 -> MAX_NAME_LENGTH + 2
	gender_t	gender;
	clientInfo_t	*ci;
...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	case EV_FOOTSTEP:
		...
		// q3as if (cg_footsteps.integer) {
		if (cg_footsteps.integer && (sta_mFootsteps.integer || es->clientNum != cg.clientNum)) { // q3as
...
//		CG_Printf( "EV_JUMP_PAD w/effect #%i\n", es->eventParm );
	...
	case EV_JUMP:
		DEBUGNAME("EV_JUMP");
		if ( sta_mJumpSound.integer || es->clientNum != cg.clientNum) // q3as - mute my jump sound
			trap_S_StartSound (NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*jump1.wav" ) );
		break;
	case EV_TAUNT:
		DEBUGNAME("EV_TAUNT");
...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
	...
	case EV_LIGHTNINGBOLT:
		...
		// q3as CG_LightningBoltBeam(es->origin2, es->pos.trBase);
		as_lightningBoltBeam(es->origin2, es->pos.trBase, ci );	// q3as
		break;
#endif
	case EV_SCOREPLUM:
	...
	case EV_MISSILE_MISS:
		...
		ByteToDir( es->eventParm, dir );
		// q3as CG_MissileHitWall( es->weapon, 0, position, dir, IMPACTSOUND_DEFAULT );
		CG_MissileHitWall( es->weapon, es->otherEntityNum, position, dir, IMPACTSOUND_DEFAULT );	// q3as
		break;

	case EV_MISSILE_MISS_METAL:
		DEBUGNAME("EV_MISSILE_MISS_METAL");
		ByteToDir( es->eventParm, dir );
		// q3as CG_MissileHitWall( es->weapon, 0, position, dir, IMPACTSOUND_METAL );
		CG_MissileHitWall( es->weapon, es->otherEntityNum, position, dir, IMPACTSOUND_METAL );		// q3as
		break;
